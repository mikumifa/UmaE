...
for track_id, track in points_data.items():
    for course_id, course in track["courses"].items():
        points = course.get("slopes", [])
        if not points:
            continue
...
# 保存新文件
with open("src\components\data\course_data.json", "w", encoding="utf-8") as f:
    json.dump(points_data, f, ensure_ascii=False, indent=4)

print("转换完成，生成 course_data.json")
